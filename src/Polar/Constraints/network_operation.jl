...
function pullback_network(polar::PolarForm{T, VI, VT, MT}, nbatch; with_stack=with_stack) where {T, VI, VT, MT}
    nnz = length(polar.topology.ybus_im.nzval)
    nx = get(polar, NumberOfState())
    nbus = get(polar, PS.NumberOfBuses())
    # Intermediate state
    intermediate = (
        ∂inj = MT(undef, 2*nbus, nbatch),
        ∂edge_vm_fr = MT(undef, nnz, nbatch),
        ∂edge_va_fr = MT(undef, nnz, nbatch),
        ∂edge_vm_to = MT(undef, nnz, nbatch),
        ∂edge_va_to = MT(undef, nnz, nbatch),
    )
...
function AutoDiff.TapeMemory(
    ...
    return pullback_network(polar, 1; with_stack=with_stack)
end
