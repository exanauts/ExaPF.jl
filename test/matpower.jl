...
@testset "Power flow 9 bus case" begin
    ...
    x_sol = [0.16875136481876485, 0.0832709533581424,
             ...
             -0.07592061900861094, 0.9870068781579537, 0.9754722045044448,
             1.003375449839184, 0.9856449067439345,
             0.996185273317625, 0.9576210937650547,
            ]

    @test convergence.n_iterations == 4
    @test isapprox(x_sol, xk, atol=1e-7)

    c = ExaPF.cost_production(polar, x, u, p)
end
...
@testset "Power flow 14 bus case" begin
    ...
    jx, ju = ExaPF.init_ad_factory(polar, x, u, p)
    xk, conv = ExaPF.powerflow(polar, jx, x, u, p, verbose_level=0)

    @test conv.n_iterations == 2
    @test isapprox(conv.norm_residuals, 1.3158e-10, rtol=1e-4)

    vmag, vang, pinj, qinj = ExaPF.get_network_state(polar, xk, u, p)
